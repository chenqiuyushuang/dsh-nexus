...
var NOT_NEGATION_RE = /不(?:错|少|同|仅|但|过|断|如|光|只|久|锈钢)/g;
  ...
  return {
    ...
    async retrieve(input, _signal) {
      ...
      if (versionedHit !== void 0) {
        const rankedHit = versionedHit.map((id, index) => {
          const atom = input.store.get(id);
          return atom === void 0 ? void 0 : { ...atom, score: (versionedHit.length - index) / versionedHit.length, source: "text" };
        });
        return rankedHit.filter((entry) => entry !== void 0);
      }
...
function installNexusWeb(ctx, facility, options = {}) {
  ...
  route("/nexus/api/memory/purge", async (req, res) => {
    ...
    let purged = 0;
    for (const id of ids) {
      if (store.getAtom(id) === void 0) continue;
...
function installTools(ctx, facility, resolved) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args) => {
      ...
      return hits.length === 0 ? "\u65E0\u7ED3\u679C" : hits.map((atom, index) => index + 1 + ". [" + atom.slot + "\xB7" + atom.kind + "] " + atom.subject + "\uFF1A" + atom.statement + " (" + (atom.confidence * 100).toFixed(0) + "% conf, " + atom.id + ")").join("\n");
    }
  }));
  ctx.tools.register(defineTool({
    name: "memory_read",

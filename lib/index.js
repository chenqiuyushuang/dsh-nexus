...
function installNexusWeb(ctx, facility, options = {}) {
  ...
  route("/nexus/api/memory", async (req, res) => {
    ...
    const items = [...store.atomEntries()].map(([, a]) => a).filter((a) => scope === "" || a.scope === scope).filter((a) => status === "" || a.status === status).filter((a) => reviewNote === "" || a.reviewNote === reviewNote).filter((a) => query.length === 0 || (a.subject + a.statement).toLowerCase().includes(query.toLowerCase())).sort((a, b) => b.updatedAt - a.updatedAt).slice(0, limit);

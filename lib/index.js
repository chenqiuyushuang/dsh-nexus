...
async function runLifecycle(store, now = Date.now(), force = false, recallMax = RECALL_LOG_MAX) {
  ...
  const pendingSpan = PENDING_TTL_DAYS * 864e5;
  for (const [id, atom] of [...store.atomEntries()]) {
    if (atom.status === "pending") {
      ...
      await store.updateAtom(id, (current) => current.status !== "pending" ? current : { ...current, status: "archived", reviewNote: "pending-expired", updatedAt: now });
      expired += 1;
      continue;
    }
...
var USER_TRIGGER_RE = new RegExp("(?:" + TRIGGER_PHRASES.join("|") + ")", "i");
...
var QUESTION_RE = /(?:[？?]\s*$|[吗呢][？?]?\s*$|^(?:你|您)[^，。！？]{0,16}(?:吗|呢)[？?]?$|(?:是不是|有没有|会不会|能不能|可不可以|要不要|好不好|行不行)|(?:什么|怎么|为什么|哪个|哪里|多少)[^，。！？]{0,12}[？?]|还是[^，。！？]{1,12}(?<![吧了的啊嘛])$|(?:是|对|好|行|可以|中)吧[？?]?\s*$)/i;
...
var TRIGGER_CLAUSE_HEAD_RE = new RegExp("(?:^|[\uFF0C\u3002\uFF01\uFF1F\uFF1B\uFF1A\\s])(?:\u8BF7|\u5E2E\u6211|\u9EBB\u70E6|\u4EE5\u540E|\u4ECA\u540E|\u4E00\u5B9A\u8981|\u52A1\u5FC5|\u8BB0\u5F97|\u5207\u8BB0)?\\s*(?:" + TRIGGER_PHRASES.join("|") + ")", "i");
var SECOND_PERSON_TRIGGER_RE = /(?:你|您|是否|能否|可否|会不会|能不能)[^，。！？；]{0,6}(?:记住|记得|别忘)/;
var THIRD_PERSON_QUOTE_RE = /^(?:他|她|它|他们|她们|别人|同事|用户|对方|有人|大家)[^，。！？]{0,8}(?:说|讲|问|表示|认为|提到|写道|要求|强调)/;
...
function extractFromTrigger(text, projectRef) {
  ...
  if (THIRD_PERSON_QUOTE_RE.test(text.trim())) return void 0;
...
var TEMPORARY_RE = /(?:暂时|临时|先这样|回头再说|待会再|下次再说|今天先|先记一下|就这一次|这事儿先)/;
var DERIVABLE_RE = /(?:从|看|读|翻)\s*(?:package\.json|tsconfig|\.env|源码|代码|配置文件)[^，。]{0,10}(?:看|可知|知道|能看出|写着)|^(?:package\.json|tsconfig|\.env)\s*(?:里|中)/;
...
var NexusFacility = class _NexusFacility {
   ...
   */
  async recordValueShadow(store, candidate) {
    try {
   ...
   */
  async logReject(store, source, reason, candidate, context) {
    try {
...
var nexusStateSchema = zod2.object({
   ...
   */
  panelMode: zod2.union([zod2.literal("readwrite"), zod2.literal("readonly"), zod2.literal("paused")]).optional()
});
...
var SessionModeControl = class {
  ...
  globalMode;
  get(sessionId) {
    return this.overrides.get(sessionId) ?? this.globalMode ?? this.globalDefault;
  }
  set(sessionId, mode) {
  ...
  global() {
    return this.globalMode ?? this.globalDefault;
  }
};
...
function shouldReinjectAfterCompaction(eventType) {
...
var rulesFileCache = /* @__PURE__ */ new Map();
function rulesTextFor(cwd) {
  if (cwd === void 0 || cwd === "") return void 0;
  const cached = rulesFileCache.get(cwd);
  if (cached !== void 0) return cached === "" ? void 0 : cached;
  let text = "";
  for (const name2 of ["AGENTS.md", "CLAUDE.md"]) {
    try {
      text += readFileSync(join3(cwd, name2), "utf8") + "\n";
    } catch {
    }
  }
  ...
  async function handleSessionEvent(session, event) {
    try {
      ...
      if (type === "user/message") {
        ...
        if (candidate !== void 0 && mode !== "pause") {
...
var HttpEncoder = class {
  ...
  async encode(text) {
    ...
    if (!res.ok) {
      this.noteFailure();
      throw new Error("encoding endpoint " + res.status);
    }
    const data = await res.json();
    const vec = data.embeddings?.[0];
    if (vec === void 0 || vec.length !== this.config.dim) {
      this.noteFailure();
      throw new Error("encoding shape mismatch");
    }
    this.failures = 0;
    return vec;
  }
...
function summarizeToday(input) {
  ...
  const parts = ["\u4ECA\u65E5\u5199\u5165 " + String(saved) + " \u6761"];
...
function installNexusWeb(ctx, facility, options = {}) {
  ...
  route("/nexus/api/memory/create", async (req, res) => {
    ...
    const rawProject = body?.projectRef;
    const trimmedProject = rawProject === void 0 ? void 0 : String(rawProject).trim().slice(0, 300);
    const projectRef = scope !== "project" || trimmedProject === void 0 || trimmedProject === "" || trimmedProject === "unknown" ? void 0 : trimmedProject;
...
function renderShell() {
  ...
  for (const file of ["../lib/nexus.html"]) {
    try {
      shellCache = readFileSync2(join4(here, file), "utf8");
      return shellCache;
    } catch (error) {
...
function installTools(ctx, facility, resolved) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args) => {
      ...
      if (args.trace === true) {
        const chain = [];
        const seen = /* @__PURE__ */ new Set([atom.id]);
        let prior = atom.supersedes;
        while (prior !== void 0 && !seen.has(prior) && chain.length < 20) {
          seen.add(prior);
          const earlier = store.getAtom(prior);
          if (earlier === void 0) {
            chain.unshift("\u2190 " + prior + "\uFF08\u7F3A\u5931\uFF09");
            break;
          }
        ...
        let later = atom.supersededBy;
        while (later !== void 0 && !seen.has(later) && chain.length < 40) {
          seen.add(later);
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args) => {
      ...
      for (const id of ids) {
        ...
        await store.updateAtom(id, (current) => good ? { ...current, weight: Math.min(20, current.weight + 1), updatedAt: Date.now() } : { ...current, weight: Math.max(1, current.weight - 1) });
        changed += 1;
      }
...
function diagnose(input) {
  ...
  info("\u8BB0\u5FC6\u5E93\uFF1A\u6D3B\u8DC3 " + String(active) + " \xB7 \u5F85\u786E\u8BA4 " + String(pending) + " \xB7 \u51B2\u7A81 " + String(conflicts) + " \xB7 \u5DF2\u5F52\u6863 " + String(archived) + " \xB7 \u5DF2\u53D6\u4EE3 " + String(superseded));
  ...
  else info("\u4ECA\u65E5\uFF1A\u5199\u5165 " + String(input.today.saved) + " \u6761 \xB7 \u62D2\u6536 " + String(input.today.rejected) + " \u6761 \xB7 \u6CE8\u5165 " + String(input.today.injections) + " \u6B21");
  if (input.valueGate !== void 0) {
    const gate = input.valueGate;
    const total = gate.accept + gate.review + gate.reject;
    if (total > 0) info("\u4EF7\u503C\u95E8\uFF08\u5F71\u5B50\u671F\uFF0C\u53EA\u8BB0\u5F55\u4E0D\u62E6\u622A\uFF09\uFF1A\u63A5\u53D7 " + String(gate.accept) + " \xB7 \u5F85\u8BAE " + String(gate.review) + " \xB7 \u4F4E\u4EF7\u503C " + String(gate.reject));
  }
...
// src/commands.ts
var USAGE = "/memory list|search <q>|show <id>|edit <id> <\u65B0\u9648\u8FF0>|delete <id...>|confirm <id...>|reject <id...> [\u539F\u56E0]|conflict [keep-new|keep-old|keep-both <id...>|--all]|cost|doctor|session [read-write|write-only|pause]";
async function resolveConflicts(facility, ids, strategy) {
  const store = await facility.store();
  let done = 0;
  for (const id of ids) {
    const atom = store.getAtom(id);
    if (atom === void 0 || atom.status !== "needs-review") continue;
    const now = Date.now();
    if (strategy === "keep-both") {
      await store.updateAtom(id, (current) => ({ ...current, status: "active", conflictWith: void 0, reviewedAt: now, reviewNote: "conflict:both-kept" }));
    } else if (strategy === "keep-old") {
      await store.updateAtom(id, (current) => ({ ...current, status: "archived", reviewedAt: now, reviewNote: "conflict:kept-old" }));
      await store.putReject({
        id: rejectId(),
        at: now,
        sessionId: "conflict",
      ...
      if (rivalId !== void 0) {
        ...
        if (rival !== void 0 && rival.status === "active") {
          await store.updateAtom(rivalId, (current) => ({ ...current, status: "superseded", supersededBy: id, updatedAt: now }));
        }
      }
      await store.updateAtom(id, (current) => ({ ...current, status: "active", conflictWith: void 0, reviewedAt: now, reviewNote: "conflict:kept-new" }));
    }
    done += 1;
  }
      ...
      try {
        ...
        switch (command) {
          ...
          case "edit": {
            ...
            if (id === void 0 || statement.length < 2) return { kind: "error", text: "\u7528\u6CD5\uFF1A/memory edit <id> <\u65B0\u9648\u8FF0>" };
            const store = await facility.store();
            const atom = store.getAtom(id);
            if (atom === void 0) return { kind: "error", text: "\u672A\u627E\u5230\u8BB0\u5FC6 " + id };
          ...
          case "conflict": {
            ...
            const targets = args.includes("--all") ? conflicted.map((atom) => atom.id) : args.slice(1).filter((part) => part.startsWith("nex_"));
          ...
          case "skill-compile": {
            ...
            const store = await facility.store();
            const drafts = await compileSkills(store, join5(resolved.projectionDir, "skills.draft"), minWeight);
          ...
          case "doctor": {
            ...
            const report = diagnose({
              ...
              ...store.getState().valueGateShadow !== void 0 ? { valueGate: store.getState().valueGateShadow } : {},
              today,
              extractorLlm: store.getState().extractorLlm !== void 0,
...
function apply(ctx, config) {
  ...
  if (resolved.webui) {
    ctx.inject(["webServer"], (webCtx) => installNexusWeb(webCtx, facility, {
      ...
      modes: { global: () => modes.global(), setGlobal: (mode) => {
        modes.setGlobal(mode);
      } }
    }));
  }

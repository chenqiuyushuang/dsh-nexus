...
- **反死机制**：每条机制必须有指标 + 回归测试，否则不上线 ✅ **两个数字现在都由机器现算**（不写死在这里）：状态表末尾的「零测试引用的模块」与「零调用点的导出」两个小节。当前零调用点的导出只剩一个测试辅助函数；曾经的三个违反项都已收口 —— `runLifecycle`（0.8.9 接到 `Facility.tickLifecycle`）、`linkCluster`（0.8.12 接到 integrator，聚类成员建 semantic 边）、`sessionToPanelMode`（模式映射收敛到 `src/modes.ts` 后 web-ui 与面板共用）；

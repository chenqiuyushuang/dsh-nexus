...
const proc = spawn(chrome, ['--headless=new', '--no-sandbox', '--disable-gpu', '--disable-crash-reporter', '--remote-debugging-port=' + String(port), '--user-data-dir=' + root + '_shots/chrome-profile-debug3', '--window-size=1440,1000', 'about:blank'], { stdio: 'ignore' })
...
await new Promise((r) => setTimeout(r, 1200))
const diag = await evaluate("(() => { const f = document.querySelector('iframe[title=\"Nexus 记忆面板\"]'); const d = f.contentDocument; const el = d.querySelector('.nx-b'); const cs = getComputedStyle(el); const rules = [...d.styleSheets].flatMap((s) => { try { return [...s.cssRules].map((r) => r.cssText) } catch { return [] } }).filter((t) => t.includes('panel-open')); return { cls: el.className, matches: el.matches('.nx-b.panel-open'), bg: cs.backgroundColor, radius: cs.borderTopLeftRadius, bw: cs.borderTopWidth, bc: cs.borderTopColor, display: cs.display, sheetRules: rules.slice(0, 3).map((t) => t.slice(0, 150)) } })()")
